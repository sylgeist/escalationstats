                    ...
                    if 'replies' in message.keys():
                        for reply in message['replies']:
                            if reply['user'] == cloudopsuser:
                                ...
                                #print(f'Start: {start.strftime("%Y-%m-%d %H:%M:%S")} First Reply: {first_reply.strftime("%Y-%m-%d %H:%M:%S")} Response Time: {response_time:.1f} Link: {permalink(message, channel_id)}')
                                break
